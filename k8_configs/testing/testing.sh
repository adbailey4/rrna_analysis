...
OUTPUT_BUCKET_TMP="bailey-k8s/rrna_experiments/testing/"
...
if [[ "$unsupervised" == "true" ]]
  then
    ...
    IVT_TRAIN_POS="\"/data/reference/unsupervised_mod_files/yeast_18S_25S_min_prob_${FREQ_THRESHOLD}_unsupervised.positions\""
  else
    NATIVE_TRAIN_POS="\"/data/reference/yeast_18S_25S_modified.positions\""
    IVT_TRAIN_POS="\"/data/reference/yeast_18S_25S_canonical.positions\""
fi
...
run_training_routine() {
  ...
  do
    ...
    then
      ...
      aws s3 cp --no-progress testing.sh s3://"$OUTPUT_BUCKET"
